...
| [Phase-8b-Migration.md](./Phase-8b-Migration.md) | Settings-decoupling follow-up: `GeneralSettings`/`UnitSettings` get the `Preferences` design-time fallback `DatabaseSettings` already had (sub-task 3, blind-safe slice) → `DynoAudioViewModelTest` re-enabled + `SettingsTest` added; sub-task 2 (Shell routes) + full `IPlatformSettings` DI refactor still deferred |
...
> **Phase 8b (follow-up):** the blind-safe, test-verifiable slice of the Settings decoupling is now done — `GeneralSettings`/`UnitSettings` gained the `Preferences` design-time fallback that `DatabaseSettings` already had, re-enabling `DynoAudioViewModelTest` (and adding `SettingsTest`). Tests now **51 passed / 0 skipped / 0 failed**. Sub-task 2 (23 `Ioc.Default` View sites / Shell routes) and the full `IPlatformSettings` DI refactor stay deferred — device-only. See [Phase-8b-Migration.md](./Phase-8b-Migration.md).
...
- [~] **Phase 8b (follow-up)** — 🔶 partial. **Done:** Settings-layer `Preferences` design-time fallback parity (`GeneralSettings` + `UnitSettings` now mirror `DatabaseSettings`) → `DynoAudioViewModelTest` re-enabled + `SettingsTest` added; **51 passed / 0 skipped / 0 failed**; MacCatalyst head 0 errors, warning set unchanged. **Deferred w/ rationale:** sub-task 2 — 23 `Ioc.Default` View sites (Shell→registered-routes needs device verification); full `IPlatformSettings` DI abstraction (instance Settings + `OnConfiguring` options-lambda, P3 wide ripple). See [Phase-8b-Migration.md](./Phase-8b-Migration.md).
