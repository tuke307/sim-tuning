...
4. **Functional bugs** (not just modernization) — iOS `Info.plist` missing mic/location usage strings (runtime crash + App Store rejection); duplicate popup→VM registration (dead `DynoCreationPopup`/`EnvironmentPopup` — CommunityToolkit `PopupService` is first-wins, so `VehiclePopup`/`PortTimingPopup` are the live pair; **fixed Phase 6**); `DatabaseContext` registered as Singleton.
...
- **Popups**: 4 popups (`VehiclePopup`, `DynoCreationPopup`, `PortTimingPopup`, `EnvironmentPopup`) registered via `AddTransientPopup`. **Bug (direction corrected in Phase 6):** `MauiProgram.cs:106-109` maps two popups to the same VM (`VehiclePopup`+`DynoCreationPopup`→`VehiclesViewModel`; `PortTimingPopup`+`EnvironmentPopup`→`PortTimingViewModel`). CommunityToolkit.Maui `PopupService` registers the VM→view map with **`TryAdd` (first registration wins)**, so `VehiclePopup`+`PortTimingPopup` are the *live* popups and `DynoCreationPopup`+`EnvironmentPopup` are the **dead** ones (never shown). *(Phase-1 originally asserted the reverse — that the second registration overwrites the first — which is wrong; see Phase 6.)*
...
2. **Duplicate popup registration** — two popup views registered per VM type (`VehiclePopup`+`DynoCreationPopup`→`VehiclesViewModel`; `PortTimingPopup`+`EnvironmentPopup`→`PortTimingViewModel`). CommunityToolkit `PopupService` uses `TryAdd` (**first wins**), so `DynoCreationPopup`/`EnvironmentPopup` are the dead/unreachable pair. **Fixed in Phase 6** (dead pair deleted). *(Phase-1 originally asserted the opposite direction — corrected.)*
