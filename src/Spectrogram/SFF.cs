...
namespace Spectrogram
{
    ...
    public class SFF
    {
        ...
        public SKBitmap GetBitmap(Colormap? cmap = null, double intensity = 1, bool dB = false)
        {
            var _cmap = cmap ?? Colormap.viridis;
            return Image.GetBitmap(Ffts, _cmap, intensity, dB);
        }
        ...
        public void Load(string filePath)
        {
            ...
            // recording start time - no longer stored in the SFF file DateTime dt = new
            // DateTime(bytes[74] + 2000, bytes[75], bytes[76], bytes[77], bytes[78],
            // bytes[79]);
        ...
        public void Save(string filePath)
        {
            ...
            // source file date and time dont store this because it makes SFF files
            // different every time they are generated header[74] =
            // (byte)(DateTime.UtcNow.Year - 2000); header[75] =
            // (byte)DateTime.UtcNow.Month; header[76] = (byte)DateTime.UtcNow.Day;
            // header[77] = (byte)DateTime.UtcNow.Hour; header[78] =
            // (byte)DateTime.UtcNow.Minute; header[79] = (byte)DateTime.UtcNow.Second;
